#!/bin/bash
# 🔄 Neo4j Restore MCP - Restauração Segura de Backups
# Versão atualizada para estrutura src/ e backups temáticos
...
THEMATIC_DIR="${PROJECT_ROOT}/memory-backups-thematic"
...
echo -e "${CYAN}╔══════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║     🔄 Neo4j Restore MCP - Sistema Seguro   ║${NC}"
echo -e "${CYAN}╚══════════════════════════════════════════════╝${NC}\n"
...
echo -e "${PURPLE}[Backups MCP Consolidados]${NC}"
if [ -d "$BACKUP_DIR" ]; then
    ls -lh "$BACKUP_DIR"/SECURE_MCP_*.zip 2>/dev/null | tail -5 | while read line; do
        echo -e "  ${GREEN}→${NC} $(basename $(echo $line | awk '{print $NF}'))"
    done || echo -e "  ${YELLOW}Nenhum backup MCP encontrado${NC}"
else
    echo -e "  ${YELLOW}Diretório não existe${NC}"
fi

echo -e "\n${PURPLE}[Backups Temáticos]${NC}"
if [ -d "$THEMATIC_DIR" ]; then
    ls -lh "$THEMATIC_DIR"/THEME_*.json 2>/dev/null | tail -10 | while read line; do
        echo -e "  ${GREEN}→${NC} $(basename $(echo $line | awk '{print $NF}'))"
    done || echo -e "  ${YELLOW}Nenhum backup temático encontrado${NC}"

    # Mostrar índice mestre se existir
    if [ -f "$THEMATIC_DIR/MASTER_INDEX.json" ]; then
        echo -e "\n  📚 ${CYAN}Índice Mestre disponível${NC}"
        python3 -c "
import json
with open('$THEMATIC_DIR/MASTER_INDEX.json') as f:
    idx = json.load(f)
    print(f\"     • Temas: {idx.get('total_themes', 0)}\")
    print(f\"     • Nós totais: {idx.get('total_nodes', 0)}\")
"
    fi
else
    echo -e "  ${YELLOW}Diretório não existe${NC}"
...
# Script Python para restauração
cat > "$SRC_DIR/neo4j_restore_temp.py" << 'EOF'
#!/usr/bin/env python3
...
cd "$PROJECT_ROOT"
python3 "$SRC_DIR/neo4j_restore_temp.py"

# Limpar arquivo temporário
rm -f "$SRC_DIR/neo4j_restore_temp.py"

echo -e "\n${PURPLE}💡 Próximos passos:${NC}"
echo -e "   • Verificar dados: ${GREEN}./neo4j-manager-mcp.sh${NC}"
echo -e "   • Fazer novo backup: ${GREEN}./neo4j-backup-mcp.sh${NC}"